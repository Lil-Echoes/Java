...
//	콘솔창(웹)으로부터 명령어를 전달받아서 Service를 통하여 실행하고
//	그 결과를 콘솔창(웹 화면)에 알려준다
...
//	컴마(,)를 기준으로 데이터 나누어서 배열로 만들어 insert라는 메소드의 매개변수로 사용
...
//		입력한 데이터를 가지고 고객 생성
		...
		if (!customer.passChk(str[2], str[3])) {
			System.out.println("암호와 암호확인이 다릅니다");
			return;
		}
//		customerServiceImp1을 통해서 데이터를 입력시키고 그 결과를 점수로 받아온다
...
//		split(select, id) -> {select, id}
	...
	private static void list(String[] split) {
		Collection<Customer> list = cs.list();
		if(list == null) System.out.println("데이터가 없습니다");
